...
public:
    ...
    static void reverse(Linear_list<T,P> &l){
        const int size = l.size();
        for(position p = size; p != l.begin(); p = l.previous(p)) {
            value_type temp = l.read(p);
            l.erase(p);
            l.push_back(temp);
            if(l.previous(p) == l.begin()){
                l.push_back(l.read(l.begin()));
                l.erase(l.previous(p));
            }
        }
    }

    static bool palindrome(Linear_list<T,P> &l){
        bool cond = false;
        const int size = l.size();
        position iter = l.begin();
        for(position p = size; p!= l.begin(); p = l.previous(p)){
            if(l.read(iter) != l.read(p))
                break;
            else
                cond = true;
            if(l.previous(p) == l.begin())
                if(l.read(l.next(iter)) != l.read(l.previous(p)))
                    break;
                else
                    cond = true;
        }
