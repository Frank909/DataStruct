...
// classe Lista
template< class T > class List_vector : public Linear_list<T, int>{
public:
...
// costruttore
template< class T >
List_vector< T >::List_vector() : array_dimension_(10){
    this->create();
}

// costruttore
template< class T >
List_vector< T >::List_vector(int dim) : array_dimension_(dim){
    this->create();
}
...
List_vector< T >::List_vector(const List_vector<T>& Lista) :
        array_dimension_(Lista.array_dimension_), length_(Lista.length_), elements_(new T[array_dimension_]){
...
template<class T>
void List_vector<T>::push_front(const value_type &e) {
    insert(e, begin());
}


template<class T>
void List_vector<T>::push_back(const value_type &e) {
    position p = begin();
    for(; !end(p); p = next(p));
    insert(e, p);
}
...
template <class T>
void List_vector<T>::pop_front(){
    erase(begin());
}

template <class T>
void List_vector<T>::pop_back(){
    position p = begin();
    for(; !end(next(p)); p = next(p));
    erase(p);
}

template <class T>
void List_vector<T>::clear(){
    for(position p = begin(); !end(p); erase(p));
}
